...
@router.post("/direct_send", response_model=bool)
async def direct_send(sessionid: str = Form(...), 
                                     text: str = Form(...), 
                                     user_ids: List[int] = Form(...),
                                     clients: ClientStorage = Depends(get_clients)) -> DirectMessage:
    cl = clients.get(sessionid)
    return cl.direct_send(text, user_ids)